...
async function runPreflight(options: { input: Exclude<ResolvedInput, { kind: 'artifact' }>; flags: ParseFlags; mode: PreflightMode; inspector?: NodeDocumentInspector; stored?: DeckProbeReport }): Promise<PreflightRun> {
  ...
  try {
    ...
    if (options.mode === 'strict') throw DeckParseError.input(`DeckProbe preflight failed to run: ${message}`, { hint: 'Retry with preflight validate/off.', cause: error });
...
function expectedParser(input: Exclude<ResolvedInput, { kind: 'artifact' }>, engine: NonNullable<CommonFlags['engine']>): { name: string; major: number } | undefined {
  if (engine === 'cloud') return { name: 'deckflow-cloud', major: 1 };
  if (engine === 'auto' || input.kind === 'link') return undefined;
  if (input.taskType === 'pdf.pdfParse') return { name: 'pdf-lite-parse', major: 0 };
