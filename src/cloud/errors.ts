...
import type { MetadataValue } from './contracts.js';
...
function extractRequestIdFromHeaders(headers: unknown): string | undefined {
  ...
  if (typeof (headers as { get?: ((name: string) => unknown) | undefined }).get === 'function') {
  ...
  if (typeof (headers as { toJSON?: ((asStrings?: boolean) => Record<string, unknown>) | undefined }).toJSON === 'function') {
...
export function isRetriableError(error: unknown): boolean {
  if (typeof error === 'object' && error !== null && (error as { isAxiosError?: (boolean) | undefined }).isAxiosError) {
