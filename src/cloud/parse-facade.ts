...
import type { DeckTask, TaskDownloadOptions, TaskUploadInput, TaskUploadOptions, WaitForTaskOptions } from './contracts.js';
...
export interface ParseOptions {
  ...
  parseProfile?: ('fast' | 'balanced' | 'quality') | undefined;
...
export type ConvertRef = { irKey: string; taskId?: (never) | undefined } | { taskId: string; irKey?: (never) | undefined };
...
export interface ConvertOptions {
  ...
  onTask?: ((task: DeckTask) => void) | undefined;
  /** 目标 View，默认 markdown */
  to?: (ConvertTarget) | undefined;
  /** 空间 id */
  spaceId?: (string) | undefined;
  /** 等待任务完成的选项 */
  wait?: (WaitForTaskOptions) | undefined;
  /** pdf：markdown 是否写入逐元素溯源注释，默认 false */
  markdownMeta?: (boolean) | undefined;
  /** 分页格式（pptx / keynote）：是否额外返回逐页数组 */
  markdownPages?: (boolean) | undefined;
  /** 渲染失败时抛错；默认 false，容错返回 `markdownError` */
  markdownStrict?: (boolean) | undefined;
}
...
const nameForRouting = (source: ParseFileSource): string => {
  ...
  if (typeof source.file === 'object' && source.file !== null && 'input' in source.file) {
    const nested = source.file as { input: unknown; name?: (string) | undefined };
    if (nested.name) return nested.name;
    if (typeof nested.input === 'string') return nested.input;
  }
  const maybeNamed = source.file as { name?: (string) | undefined };
