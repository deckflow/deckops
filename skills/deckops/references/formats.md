...
command's `--help` for live flags. The table below records the DeckOps 2.5.1 behavior that
...
| --- | --- | --- |
| `--profile` | PDF | `balanced` works locally. `fast` and `quality` require cloud parsing in 2.5.1. |
...
| --- | --- | --- |
| `--to markdown` | Every convertible artifact/source | Markdown is the only target in 2.5.1. |
| `--anchors` | PDF | Emits provenance comments carrying DeckIR node IDs. |
| `--split-pages` | PPTX and Keynote | Use artifact conversion for reliable per-page file materialization in 2.5.1. |
