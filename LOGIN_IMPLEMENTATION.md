...
`deckops login` 命令使用**本地服务器回调**方式实现自动登录，类似于 GitHub CLI、AWS CLI 等工具。

> 说明：本文件描述的是后端支持 `DeckOps` CLI 登录流程时的实现思路。关于项目定位、Key 权限边界与云端权限模型，请优先结合 [README.md](README.md) 与 [docs/CLOUD_AUTH_MODEL.md](docs/CLOUD_AUTH_MODEL.md) 一起阅读。
...
A: 在这种情况下，可以提供备用方案：
1. 显示登录 URL，让用户手动打开
2. 登录成功后，显示 token 让用户手动执行 `deckops config set-token <token>`
