...
Deckops 是 [Deckflow](https://app.deckflow.com) 的命令列工具，用於上傳檔案、建立非同步任務，以及管理設定與任務狀態。底層透過 `@deckops/sdk` 呼叫 Deckflow API。
...
設定檔預設儲存在 `~/.deckops/config.json`。測試或隔離環境可透過環境變數指定目錄：
...
設定 workspace / space ID。部分命令需要 space 才能建立任務。
...
自訂 API 根位址（預設 `https://app.deckflow.com/v1`）。
...
透過瀏覽器 OAuth 流程登入，並將 token 寫入本機設定。
...
指定 `-o` 時，即使未明確傳 `--wait`，也會等待任務完成。

## 常見問題

**Q: 提示 Space ID missing？**

先執行 `deckops login`，或透過 `deckops config set-space <space-id>` 手動設定。
...
- Monorepo 根目錄：[README.md](../../README.zh-TW.md)
- SDK 文件：[@deckops/sdk](../../sdks/nodejs/README.zh-TW.md)
